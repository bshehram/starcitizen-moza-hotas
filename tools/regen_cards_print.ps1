<#
  regen_cards_print.ps1 - PRINT-FRIENDLY reference cards (split-diagram layout).

  Produces TWO cards sized to fill a US-Letter sheet in LANDSCAPE (11 x 8.5 in,
  aspect 1.294) with large fonts, alongside the originals (never touched):

      cards/MOZA_MHG_AB6_ref_print.png / .svg   (js1: MHG grip 1-29 + AB6 base 49-62)
      cards/MOZA_MTQ_ref_print.png     / .svg   (js2: MTQ throttle 1-65 + axes)
  ...
  WHY THIS LAYOUT
    ...
    grip head; AB6 = base). Shrunk to fit a card those sub-views get tiny. Here each
    composite is SPLIT into its sub-views (cropped from the source PNG, see the crop
    rectangles below), then the views are STACKED vertically down the left column at
    a much larger size so the button numbers are actually legible. The button# ->
    function table fills the right side in two columns.

  PRINTING
    Print "Fit to page" / 100%, landscape. Each card is already the page aspect, so
    it fills the sheet edge to edge. Output is 2x (~250-340 DPI per card).

  KEEP IN SYNC  <-- IMPORTANT
    The $AB6 / $MHG_* / $MTQ_* data arrays below are COPIES of the ones in
    regen_cards.ps1 (the wide on-screen cards). Source of truth is MOZA.xml. When a
    binding changes, edit the matching row in BOTH scripts (see CLAUDE.md sec 8.5)
    and re-run both.
  ...
  RUN (from the project root):
      powershell -ExecutionPolicy Bypass -File .\tools\regen_cards_print.ps1
#>
...
if (-not $chrome) { throw "regen_cards_print: no Chrome/Edge found to render SVG -> PNG." }
...
function New-Card($cfg){
  ...
  } else { throw "regen_cards_print: render failed for $($cfg.Name)." }
}
...
# ================================================================ AB6 (js1 49-62)
# NOTE: variant labels here are shortened vs regen_cards.ps1 to fit the narrow
# 40%-width legend columns - same bindings, terser cheat-sheet wording.
$AB6 = @(
...
New-Card @{ Name='MOZA_MHG_AB6_ref_print'; F=1.18; CanvasW=1860; CanvasH=1437;
...
New-Card @{ Name='MOZA_MTQ_ref_print'; F=1.15; CanvasW=1860; CanvasH=1437;
...
Write-Host "Done -> MOZA_MHG_AB6_ref_print, MOZA_MTQ_ref_print (.png + .svg) in $cardDir"
