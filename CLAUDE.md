...
| `js2_button63` | MTQ throttle dial — fwd/CW click | `v_view_dynamic_zoom_rel_in` + `v_view_zoom_in` | Zoom **in** — both vehicle zoom actions fire ("shotgun" — whichever works, works). Dynamic zoom zooms the **cockpit**; the 3rd-person one is dead but harmless. |
| `js2_button64` | MTQ throttle dial — back/CCW click | `v_view_dynamic_zoom_rel_out` + `v_view_zoom_out` | Zoom **out** — same shotgun. (The other 3 zoom pairs — `zoom_*`/`spectate_*`/`pc_*` — are on-foot/spectator/interaction, can't fire from a seat, so excluded.) |
