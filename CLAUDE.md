...
| `js2_button63` | MTQ throttle-lever dial — fwd/CW click | `v_view_dynamic_zoom_rel_in` → *Dynamic zoom in (rel.)* | Zoom the view FOV **in**, one step per click — **first-person cockpit only** (3rd-person external camera has no dial-bindable zoom in SC). Replaced `v_view_zoom_in` (3rd-person camera-distance, did nothing even in 3rd person). |
| `js2_button64` | MTQ throttle-lever dial — back/CCW click | `v_view_dynamic_zoom_rel_out` → *Dynamic zoom out (rel.)* | Zoom the view FOV **out**, one step per click (cockpit). |
