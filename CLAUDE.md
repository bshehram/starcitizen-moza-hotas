...
| Input | Control | Action → In-game label | What it does |
| --- | --- | --- | --- |
| `js2_button27` | MTQ **toggle B** up | `v_lights_on` → *Headlights On* | Forces exterior/interior lights **on**. **Also fires `v_light_amplification_on` (LAMP on)** — one "see in the dark" throw (see Mode switching & LAMP). |
| `js2_button28` | MTQ **toggle B** down | `v_lights_off` → *Headlights Off* | Forces lights **off**. **Also fires `v_light_amplification_off` (LAMP off)** — one "go dark" throw. |
...
| `js2_button4` | MTQ keypad **"A4"** | `v_ifcs_toggle_esp` → *ESP Toggle* | Enhanced Stick Precision — softens input near a target to reduce overshoot. On A4 (a momentary button); moved off the throttle dial, where rolling the wheel re-toggled it on every click. A4 freed by removing the LAMP toggle. *(Proximity-assist toggle dropped — confirmed **no-op** in 4.x, tested in 315p + F8C; left at its default of on.)* |
...
| `js2_button27` | MTQ **toggle B** up (with Lights On) | `v_light_amplification_on` → *LAMP On* | Canopy night-vision **on** — fires alongside `v_lights_on` on the same throw ("see in the dark"). LAMP-equipped ships only. |
| `js2_button28` | MTQ **toggle B** down (with Lights Off) | `v_light_amplification_off` → *LAMP Off* | Canopy night-vision **off** — fires alongside `v_lights_off`. Moved off `js1_button59` (AB6 left "Slider" bottom, now free); the old A4 LAMP toggle was removed (A4 = ESP now). |
...
| `js2_button63` | MTQ throttle dial — fwd/CW click | `view_fov_in` → *Advanced Camera: Increase FoV* | **Experimental** — testing whether Advanced-Camera FoV gives 3rd-person zoom. May require holding the camview modifier (`view_enable_camview_mode`), in which case a bare dial roll won't do anything. |
| `js2_button64` | MTQ throttle dial — back/CCW click | `view_fov_out` → *Advanced Camera: Decrease FoV* | **Experimental** — zoom out counterpart. If neither does anything in normal flight, the dial goes unused. |
...
5. **Unbound hardware you could still use:** **The entire AB6 wing bank is free** — all 8 buttons `js1_button49–56` — as flight/ship controls migrated onto the MTQ throttle (decoupled→57, G-Safe→16, ATC landing→js2_10, jump→js2_9; **ESP→keypad A4**; **LAMP→lights toggle 27/28**; cargo-loading + proximity-assist removed). Also open: the **MHG right hat ↓/←** (`js1_button14/15` — ↑/→ now cycle sub-target/attacker, press 16 = gimbal), the **MHG top coolie hat** (`js1_button25/26/27/28/29`, was camera look/cycle), the AB6 **"Dial" lever** (60/61/62), and the **full AB6 "Slider"** (57/58/59 — 59 freed when LAMP-off moved to the lights toggle). **MTQ (js2) is essentially full** — only the throttle dial (63/64, an **experimental** `view_fov` test that may not pan out) and a keypad spare or two. Good uses for the freed AB6 wing bank: power-allocation triage (`v_engineering_assignment_*`), shield-pip / MFD navigation, weapon-group management, or VOIP. *(MTQ otherwise fully bound: WPN hat = shield faceting, 3-pos switch = VTOL/space-brake, COM press = decoupled, A4 = ESP, both encoders + rotary knob, FLAPS/SPEEDBRAKE axes = mining/salvage.)*
