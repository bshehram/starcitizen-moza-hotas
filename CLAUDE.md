...
| `js2_button63` | MTQ throttle dial — fwd/CW click | `v_view_dynamic_zoom_rel_in` + `v_view_zoom_in` + `view_fov_in` + `view_enable_camview_mode` | Zoom **in** — "shotgun": dynamic zoom works in the **cockpit**; 3rd-person `v_view_zoom_in` is dead but harmless; `view_fov_in` + the camview modifier are an **experiment** (the modifier normally needs *holding*, so one click may not fire it — and could interfere with the working cockpit zoom). |
| `js2_button64` | MTQ throttle dial — back/CCW click | `v_view_dynamic_zoom_rel_out` + `v_view_zoom_out` + `view_fov_out` + `view_enable_camview_mode` | Zoom **out** — same shotgun + the FoV/camview experiment. (The 3 on-foot zoom pairs — `zoom_*`/`spectate_*`/`pc_*` — are excluded; can't fire from a seat.) |
