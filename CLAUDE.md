...
| `js2_button63` | MTQ throttle dial — fwd/CW click | `v_ifcs_toggle_esp` → *ESP Toggle* | Enhanced Stick Precision — softens input near a target to reduce overshoot. **Groups the flight assists on the throttle** (decoupled 57, G-Safe 16). One dial click = one toggle. Moved off `js1_button50` (now free). |
| `js2_button64` | MTQ throttle dial — back/CCW click | `v_ifcs_proximity_assist_toggle` → *Proximity Assist* | Auto-dampens thrust near surfaces for safer slow flying/landings. Boost overrides it. Paired with ESP on the dial (63 = ESP, 64 = proximity). Moved off `js1_button52` (now free). |
...
5. **Unbound hardware you could still use:** **The entire AB6 wing bank is now free** — all 8 buttons `js1_button49–56` — as flight/ship controls migrated onto the MTQ throttle (decoupled→57, ESP→63, proximity→64, G-Safe→16, ATC landing→js2_10, jump→js2_9, LAMP→keypad A4; cargo-loading removed). Also open: the **MHG right hat ↓/←** (`js1_button14/15` — ↑/→ now cycle sub-target/attacker, press 16 = gimbal), the **MHG top coolie hat** (`js1_button25/26/27/28/29`, was camera look/cycle), the AB6 **"Dial" lever** (60/61/62), and most of the AB6 **"Slider"** (57/58; 59 = LAMP off). **MTQ (js2) is essentially full** — only a couple of keypad spares left. Good uses for the freed AB6 wing bank: power-allocation triage (`v_engineering_assignment_*`), shield-pip / MFD navigation, weapon-group management, or VOIP. *(MTQ fully bound: WPN hat = shield faceting, 3-pos switch = VTOL/space-brake, COM press = decoupled, dial = ESP/proximity, both encoders + rotary knob, FLAPS/SPEEDBRAKE axes = mining/salvage — `js2_slider2`/`js2_slider1`.)*
