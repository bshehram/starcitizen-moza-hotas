...
- **Button-reference cards:** [`cards/MOZA_AB6_ref.png`](cards/MOZA_AB6_ref.png), [`cards/MOZA_MHG_ref.png`](cards/MOZA_MHG_ref.png), [`cards/MOZA_MTQ_ref.png`](cards/MOZA_MTQ_ref.png) — landscape "cheat sheets": each is the numbered manufacturer diagram (recolored to white) beside a **button# → function** lookup table. The `.svg` next to each PNG is its editable source. See [Regenerating the button-reference cards](#8-regenerating-the-button-reference-cards) (§8). These are wide (≈2:1) for on-screen viewing.
- **Print-friendly cards:** [`cards/MOZA_MHG_AB6_ref_print.png`](cards/MOZA_MHG_AB6_ref_print.png) (js1: grip 1-29 **+** base 49-62, combined) and [`cards/MOZA_MTQ_ref_print.png`](cards/MOZA_MTQ_ref_print.png) (js2 throttle) — reshaped to **US-Letter landscape (11×8.5, ratio 1.294)** with **larger fonts**, and the composite manufacturer diagram **split into its sub-views, stacked larger** down the left column so the button numbers are legible in hand. Generated by [`tools/regen_cards_print.ps1`](tools/regen_cards_print.ps1). See §8.5.
- **Card generators:** [`tools/regen_cards.ps1`](tools/regen_cards.ps1) (three wide screen cards, one per device) and [`tools/regen_cards_print.ps1`](tools/regen_cards_print.ps1) (two Letter-landscape print cards) — both rebuild from the manufacturer diagrams + the binding tables embedded in them. The two scripts hold **separate copies** of the binding tables, so re-run **both** (and edit both tables) after any binding change.
...
│   ├─ MOZA_{AB6,MHG,MTQ}_ref.{png,svg}        ← wide (~2:1) on-screen cards (one per device)
│   └─ MOZA_MHG_AB6_ref_print.{png,svg} · MOZA_MTQ_ref_print.{png,svg}  ← Letter-landscape, split+stacked diagrams
...
└─ tools/                    ← regeneration scripts (run from the project root)
    └─ refresh_keybinds_db.ps1 · regen_cards.ps1 · regen_cards_print.ps1
```
...
The three `MOZA_<device>_ref.png` cards are **printable cheat sheets**: on the left, the manufacturer's numbered diagram recolored onto a white background; on the right, a **button# → function** lookup table. You glance at a number on the diagram, find that number in the table, read what it does. They're rebuilt by one committed script — [`tools/regen_cards.ps1`](tools/regen_cards.ps1) — and there's **no hand-placement of numbers** anywhere in the process (that's the whole point; see §8.2). For the **US-Letter landscape, larger-font** variants meant for actually printing, see **§8.5** ([`tools/regen_cards_print.ps1`](tools/regen_cards_print.ps1)).
...
### 8.5 Print-friendly cards (`regen_cards_print.ps1`)

The wide `*_ref` cards above are ~2:1 — great on screen, but printed onto **US-Letter landscape (11×8.5, ratio 1.294)** the printer scales them to the page *width*, leaving big empty bands top and bottom and shrinking every label; their composite diagrams also pack several sub-views into one image, so each ends up tiny. [`tools/regen_cards_print.ps1`](tools/regen_cards_print.ps1) fixes both. It emits **two** cards authored at the **paper's own 1.294 ratio** with **larger fonts**:

| Print card | Covers | Top-left band (two views side by side) | Bottom-left band (one big view) |
| --- | --- | --- | --- |
| `MOZA_MHG_AB6_ref_print` | js1 — **grip 1-29 + base 49-62** combined | MHG grip front (head + hats) · MHG grip side | AB6 base (smaller — far fewer buttons) |
| `MOZA_MTQ_ref_print` | js2 — **throttle 1-65 + axes** | Right Module · Left Module | Throttle panel (the button-dense view) |

**Layout = a 60 / 40 split.** Left **~60%** of the sheet is the diagrams, stacked as two fixed **bands (quadrants)**: the **two small sub-views side by side on top**, the **one big sub-view below**. Right **~40%** is the lookup table in **two columns**. (Earlier iterations used 50/50 and a single vertical diagram column — the 60/40 + side-by-side bands is what the user settled on.) Rebuild (from the project root):

```powershell
powershell -ExecutionPolicy Bypass -File .\tools\regen_cards_print.ps1
```

**Print at "Fit to page" / 100%, landscape** — the image is already the page aspect, so it fills the sheet. Output resolution is 2× ≈ 330 DPI.

It reuses the recolor/render machinery from §8.2, plus these additions:

- **Split diagrams.** `Get-SubDiagram` crops a sub-rectangle out of a manufacturer PNG (and recolors it) so each view can be shown big on its own. The crop rectangles (`x0,y0,x1,y1` in source pixels) live in the script header — re-verify them only if MOZA ships new device art.
- **`Add-DiagramStack`** lays the sub-views into the left zone as a column of **bands** (`$rows`). Each band gets a height share (`HFrac`, equal by default) and holds one or more images placed **side by side**, each owning a cell = `Frac` of the zone width and fit/centred in it (optional caption underneath). To resize a view, change its `Frac` (width) — e.g. the AB6 base's `Frac` is kept below the grips' so it reads smaller. The table fills the right side in two columns; the combined card puts MHG 1-15 in column 1 and MHG 16-29 **+** the whole AB6 block in column 2.
- **`$F` per card = a global font/size scale** (`round(base*$F)` everywhere). The narrow 40% table forces a smaller `$F`: **MHG+AB6 1.18**, **MTQ 1.15**. `CanvasW:CanvasH` is held at **1.294** so the sheet is filled. Want bigger diagrams? widen the `Stack` `W` and shrink the table columns / `$F`.
- ⚠ **Abbreviated labels.** Because the 40% columns are narrow, this script's data arrays use **shorter** primary/variant/header text than `regen_cards.ps1` (e.g. headers are just `PANEL BUTTONS 1-30`, variants are `Salv: gap +` not `Salvage Beam Spacing Increase`). Same bindings, terser wording — keep that in mind when syncing the two scripts.
- ⚠ **Two things the original §8.4 script gets away with but this one can't, because it renders from a path containing spaces (`…\Program Files\…`) while the user's Chrome is usually open:**
  - **Screenshot + URL must be space-free on the command line.** `Start-Process` (PS 5.1) doesn't quote array args, so a `--screenshot=…\Program Files\…` value splits and Chrome aborts with *"Multiple targets are not supported in headless mode."* The script renders to a **space-free `$env:TEMP` path** then `Move-Item`s into `cards/`, and passes the SVG as a **`%20`-encoded `[Uri].AbsoluteUri`**.
  - **Isolated `--user-data-dir`.** A headless launch against the **default** profile collides with a running Chrome. The script points `--user-data-dir` at a per-card temp dir (and removes it after).

> **⚠ Keep the two scripts in sync.** `regen_cards_print.ps1` carries its **own copy** of the `$AB6 / $MHG_* / $MTQ_*` data arrays (a separate script so it never touches the originals). Source of truth is still `MOZA.xml`. When a binding changes you now edit the matching row in **both** `regen_cards.ps1` **and** `regen_cards_print.ps1` (per §8.3), then re-run both.
